...
# Custom timer macro to only enable on rank 0 if timings is true
macro timeit_on_master(timings_flag_expr, name_expr, ex_expr)
    quote
        if $(esc(timings_flag_expr)) && MPI.Comm_rank(MPI.COMM_WORLD) == 0
            # Ensure TimerOutputs.DEFAULT_TIMER is used explicitly if @timeit needs it
            TimerOutputs.@timeit TimerOutputs.DEFAULT_TIMER $(esc(name_expr)) begin
                $(esc(ex_expr))
            end
        else
            $(esc(ex_expr))
...
"""
ComParCPU_MPI(circ::Circ, entrada::String, eixida::String, n_com::Int;
                timings::Bool=true, decompose::Bool=true)

Implements a three-phase tensor network contraction algorithm with MPI-based
parallelization for Phase 2 (community-wise contraction).

# Arguments
- `circ::Circ`: The quantum circuit to evaluate.
- `entrada::String`: List of input qubits.
- `eixida::String`: List of output qubits.
- `n_com::Int`: Number of communities to explore/target.
- `timings::Bool=true`: If `true`, logs and prints timing results (on rank 0).
- `decompose::Bool=true`: If `true`, decomposes circuit gates into simpler gates.

# Returns
- On rank 0: `Array{ComplexF64, 0}` (the final contracted scalar).
- On other ranks: `nothing`.
"""
function ComParCPU_MPI(circ::Circ, entrada::String, eixida::String, n_com::Int;
    timings::Bool=true, decompose::Bool=true)
